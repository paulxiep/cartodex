...
- **Viewport culling** — a per-frame test skips features outside the drawn viewport (a flat lon/lat window, a globe spherical cap), so a zoomed-in map paints a fraction of the world instead of all of it. It is an optimization, never a correctness requirement: every uncertain case draws, so nothing goes missing at the dateline or the poles.
- **Lazy, multi-resolution geometry** — the base map ships as coarse/mid/fine tiers (110m/50m/10m) built from one Natural Earth 1:10m source, and the heavy river network as a light default plus a finer tier. The coarse tier draws at world-fit; a finer one is fetched only when you zoom in, with hysteresis so it does not thrash at the threshold. The tiers are self-hosted same-origin, so the CDN base dependency is dropped.
